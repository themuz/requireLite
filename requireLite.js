...
If you wish to defer loading dependent modules to run-time (vs load-time) ensure you save/use the 
requireLite.cwd (Current working directory) property at load time,
since the cwd may differ at run-time. 
Use this variable when loading modules at run-time.
NOTE: cwd INCLUDES the trailing "/"
...
function requireLite(url, callback) {
    ...
    if (module && !module.noCache) {
        ...
        requireLite.cmodule.requires[url]='cach';
    ...
    requireLite.cmodule.requires[url]='load';
...
requireLite.xhrComplete = function (module, xhr) {
    var fn,push_cwd,push_cmodule,ctype,split;
